...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    alpha = x.dot(Wx) +  prev_h.dot(Wh) +b #(N,H)

    next_h = np.tanh(alpha)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      ...
      if (t != 0):
        ...
        h_t_prev = h0
      h[:,t,:], cache_t = rnn_step_forward(x_t,h_t_prev,Wx,Wh,b)
...
def word_embedding_backward(dout, cache):
    ...
    x, W = cache 
    #x(N,T) ; dout(N,T,D);
    dW = np.zeros(W.shape)# W(V,D)
    # print(dout.shape)
    np.add.at(dW, x, dout) #W[x]位置得到了词向量，x[N,T]的每一个元素都是词典的下标，传递回词典dout[N,T]累加即可
                          # W[ x[N,T] ] += dout[ x[N,T]]
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    d_ai = di * sigmoid(a_i) * (1- sigmoid(a_i))
    d_af = df * sigmoid(a_f) * (1 - sigmoid(a_f))
    d_ao = d_o * sigmoid(a_o) * (1 - sigmoid(a_o))
    d_ag = dg * (1 - np.tanh(a_g)**2)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      ...
      if (t != 0):
        ...
        h_t_prev = h0
        c_t_prev = c0
      h[:,t,:], c[:,t,:], cache_t = lstm_step_forward(x_t, h_t_prev, c_t_prev, Wx, Wh, b)
      cache.append(cache_t)
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
      ...
      dx_t, dh_t, dc_t, dWx_t, dWh_t, db_t = lstm_step_backward(dh_t, dc_t, cache_t)
