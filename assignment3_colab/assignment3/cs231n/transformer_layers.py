...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        exp_ = torch.exp(torch.arange(0, embed_dim, 2)* (-math.log(10000)/embed_dim))#  0, 2,  4,

        pe[:,:, 0::2] = torch.sin(pos * exp_)
        pe[:,:,1::2] = torch.cos(pos * exp_)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        H = self.num_heads
        Q = self.query(query).view(N, S, H, D//H).permute(0,2,1,3) #(N, H, S, D//H)
        K = self.key(key).view(N, T, H, D // H).permute(0, 2, 1, 3) #(N, H, T, D//H)
        V = self.value(value).view(N, T, H, D // H).permute(0, 2, 1, 3) #(N, H, T, D//H)

        energy = torch.matmul(Q, K.permute(0, 1, 3, 2)) # (N,H,S,D//H) * (N,H,D//H,T)
                                                        # (N,H)--(S,D//H)*(D//H,T)
                                                        # (N,H,S,T)
                                                        #此处产生了一个(S,T)的大矩阵
        ...
        attention = self.dropout(torch.softmax(energy, dim = -1)) #(N,H,S,T)
        

        output =  torch.matmul(attention, V) #(N,H,S,D//H)
      
        output = output.permute(0, 2, 1, 3).contiguous() #(N,S,H,D//H)
            # permute()不改变数据的连续性（内存不变），只是改变了解析方式。
            # 唯一改变连续性的操作，是对tensor进行切片
            #.view()调用时，可理解为浅拷贝，仍然在原数据内存空间进行操作，只是换了解析方式

        output = output.view(N, S, D) #(N,S,D)
