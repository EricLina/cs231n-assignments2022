...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for k2 in range(2*N):
            z_i = out[k2]
            if k2 != k+N:
                Denominator_KN_K += torch.exp(sim(z_k_N, z_i) / tau)
            if k2 != k: 
                Denominator_K_KN += torch.exp(sim(z_k, z_i) / tau)


        Numerator_KN_K = torch.exp(sim(z_k_N, z_k) / tau)
        Numerator_K_KN = torch.exp(sim(z_k, z_k_N) / tau)

        total_loss += (-torch.log(Numerator_KN_K/Denominator_KN_K) -torch.log(Numerator_K_KN/Denominator_K_KN))
...
def sim_positive_pairs(out_left, out_right):
    ...
    left_norm = left_norm.unsqueeze(-2) #(N,1,D)
    right_norm = right_norm.unsqueeze(-1) #（N,D,1）
    pos_pairs = torch.matmul(left_norm, right_norm).view(-1,1) #(N,1)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_pairs = torch.cat([sim_pairs, sim_pairs], dim=0) #因为sim(k,K+N) 与 sim(K+N,k)是一样的
